...
class GemmaModel(BaseModel):
    def __init__(self, model_id="google/gemma-2b"):
        self.model_id = model_id
        self.model = None
        self.tokenizer = None

    def load(self):
        """Loads the Gemma model and tokenizer with 4-bit quantization."""
        try:
    ...
    def generate(self, prompt: str, **kwargs):
        """Generates text using the Gemma model (non-streaming)."""
    ...
    def stream(self, prompt: str, **kwargs):
        """Generates text using the Gemma model (streaming)."""
        if not self.model or not self.tokenizer:
            raise RuntimeError("Model is not loaded. Please call 'load' first.")
