...
class PoAConsensus {
    ...
    selectValidator() {
        // ... (existing validator selection logic)
    ...
    validateBlock(block, parentBlock) {
        // ... (existing block validation logic)
        ...
        if (block.height > latestBlock.height) {
            if (this.peerManager) {
                ...
                if (newChain && this.validateChain(newChain) && newChain.length > this.chain.length) {
                    ...
                    this.chain = newChain;
                    // Re-add transactions from the old chain that are not in the new one
                    ...
                    // This is a simplified approach. A real implementation would need to handle this more carefully.
    ...
    validateChain(chain) {
        // ... (existing chain validation logic)
